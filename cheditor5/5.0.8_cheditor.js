...
var GB = {
	...
	cantHaveChildren : { area:1, base:1, basefont:1, col:1, frame:1, hr:1, img:1, br:1, input:1, isindex:1, link:1, meta:1, param:1 },
...
xhtmlParse : function (node, lang, encoding, needNewLine, inside_pre) {
	...
	var mediaAlign = '';
	var reParsedValue = new Date().getTime();

	for (i = 0; i < len; i++) {
		var child = children[i];

		if (GB.browser.msie) {
			if (child.getAttribute && child.getAttribute('reparsed_attr') == reParsedValue)
				continue;
			if (child.setAttribute)
				child.setAttribute('reparsed_attr', reParsedValue);
		}
		...
		switch (child.nodeType) {
			case 1: {
				...
				if ((elemName == 'p' || elemName == 'div') && !child.hasChildNodes()) {
					child.appendChild(document.createTextNode('\u00A0'));
				}

				if (GB.browser.msie) {
					if (elemName == 'object' && !(child.canHaveChildren || child.hasChildNodes())) {
				...
				for (var j = 0; j < attrLength; j++) {
					...
					if (validAttr) {
						if (!(elemName == 'li' && attrName == 'value')) {
							xhtmlText += ' ' + attrName + "=";
							xhtmlText += (/"/.test(attrValue)) ? "'" + attrValue + "'" : '"' + attrValue + '"';
						}
					}
				...
				if (!GB.cantHaveChildren[child.tagName.toLowerCase()] || child.hasChildNodes()) {
					xhtmlText += '>';
					xhtmlText += this.xhtmlParse(child, lang, encoding, newLine, inside_pre || (elemName == 'pre') ? true : false);
					xhtmlText += '</' + elemName + '>';
				}
				else {
					var innerTxt = null;
					...
					case 'textarea' :
					case 'a' :
					case 'iframe' : innerTxt = child.innerHTML; break;
					default : xhtmlText += ' />';
					}
					
					if (innerTxt) xhtmlText += innerTxt + '</' + elemName + '>';
				}
				break;
			}
			case 3: {
...
_canHaveChildren : function (elem) {
...
getContents : function (fullSource) {
    ...
    if (this.config.outputXhtml) {
    	mydoc = this.xhtmlParse(fullSource ? this.doc : this.doc.body, this.config.xhtmlLang, this.config.xhtmlEncoding);
    }
	else {
		if (fullSource) {
