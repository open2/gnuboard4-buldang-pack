...
<tr>
    ...
    <td><input type=text name='bo_<?=$i?>_subj' value='<?=get_text($board["bo_{$i}_subj"])?>' title='여분필드 <?=$i?> 제목' style='text-align:right;font-weight:bold;'></td>
    <td><input type=text style='width:80%;' name='bo_<?=$i?>' value='<?=get_text($board["bo_$i"])?>' title='여분필드 <?=$i?> 설정값'></td>
</tr>
<? } ?>
...
<div>
    ...
    <input type=button class="btn btn-default" value='  목  록  ' onclick="document.location.href='./board_list.php?<?=$qstr?>';">&nbsp;
    <? if ($w == 'u') { ?><input type=button class="btn btn-default" value='  복  사  ' onclick="board_copy('<?=$bo_table?>');"><?}?>
</div>
