...
<div class="btn-group">
    ...
    echo "&nbsp;다운로드 포인트 합계 : " . number_format($row[sum_point]) . "점)";
    ?>
</div>
...
<table width=100% class="table table-condensed table-hover table-responsive" style="word-wrap:break-word;"1>
<colgroup width=30>
<colgroup width=80>
<colgroup width=100>
<colgroup width=''>
...
<tr class='success'>
    ...
    <td><?=subject_sort_link('a.dn_datetime')?>일시</a></td>
...
for ($i=0; $row=sql_fetch_array($result); $i++) 
{
    ...
    <tr class='list$list col1 ht center'>
        ...
        <td>$row[download_point]&nbsp;</td>
        <td>" . get_datetime($row[dn_datetime]) . "</td>
        <td>$row[dn_count] (<a href='?sfl=all_dn&stx=all_dn&bo_table=$row[bo_table]&wr_id=$row[wr_id]'>".number_format($row[bf_download])."</a>)</td>
    </tr> ";
} 
