...
// ÆäÀÌÂ¡ ÄÚµåÀÇ <div><span> ÅÂ±× ´ÙÀ½¿¡ ÄÚµå¸¦ »ðÀÔ
...
// ÆäÀÌÂ¡ ÄÚµåÀÇ </span></div> ÅÂ±× ÀÌÀü¿¡ ÄÚµå¸¦ »ðÀÔ
...
// °Ô½Ã±Û ¸®½ºÆ®¿¡ ¾²ÀÏ 1°³ÀÇ ÆÄÀÏ Á¤º¸¸¦ ¾ò´Â´Ù.
    ...
    */
    ...
    if ($board['bo_use_sideview'] && $tmp_mb_id)
        if ($sideview[$tmp_mb_id]) { // ÇÑ¹ø ºÒ·¯¿Â Á¤º¸°¡ ÀÖÀ¸¸é ±×°ÍÀ» »ç¿ë
            ...
            // °Ô½ÃÆÇ¿¡ Ãâ·ÂÇÒ ±Û¾´ÀÌÀÇ ÀÌ¸§/´Ð³×ÀÓÀÌ ¹Ù²î¾úÀ» ¶§ ¾î¶»°Ô µÉ±î¿ä? ¸ô·¡ ´Ð³×ÀÓ ¹Ù²Ù°í Àå³­Ä¡¸é ¾îÂî µÉ±î¿ä?
            ...
            // ÀÌ¸ÞÀÏ sideview´Â ¸ÞÀÏÀÎÁõÀ» ¾²°í ÀÎÁõµÈ °æ¿ì¿¡¸¸ º¸¿©Áö°Ô ÇÕ´Ï´Ù.
...
function get_sideview($mb_id, $name="", $email="", $homepage="")
{
    ...
    // redis ¼¼¼Ç°ü¸®.
    // °Ô½ÃÆÇ Á¤º¸°¡ ÀÖÀ» ¶§¸¸ »ý¼ºÇÏ°í ÀÌ¿Ü¿¡´Â »ý¼º ¾ÈÇÕ´Ï´Ù. sideview°¡ ¼¯ÀÌ´Â °ÍÀ» ¸·±â À§ÇØ¼­
    /*
    ...
    */
    ...
    //·Î±×ÀÎ È¸¿ø¸¸ »çÀÌµåºä°¡ º¸ÀÌ°í, ³ª¸ÓÁöÀÇ °æ¿ì ´Ð³×ÀÓÀÌ³ª ÀÌ¸§¸¸ º¸ÀÌµµ·Ï
    if ($member['mb_id']) {
...
// mysql_real_escape_string ÀÇ alias ±â´ÉÀ» ÇÑ´Ù.
...
function check_device($device)
{
    ...
    if ($device=='pc' && G4_IS_MOBILE) {
        alert('PC Àü¿ë °Ô½ÃÆÇÀÔ´Ï´Ù.', G4_URL);
    } else if ($device=='mobile' && !G4_IS_MOBILE) {
        alert('¸ð¹ÙÀÏ Àü¿ë °Ô½ÃÆÇÀÔ´Ï´Ù.', G4_URL);
    }
}
...
function html_end()
{
    ...
    // ¹öÆÛÀÇ ³»¿ë¿¡¼­ body ÅÂ±× Áß°£ÀÇ ¿ÜºÎ css ÆÄÀÏÀ» CAPTURE ÇÏ¿© head ÅÂ±×·Î ÀÌµ¿½ÃÄÑÁØ´Ù.
